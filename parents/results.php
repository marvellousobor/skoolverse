...
if ($session_id > 0 && $term_id > 0) {
    $stmt = $conn->prepare("SELECT sr.subject_id, s.subject_name, sr.score FROM student_results sr JOIN subjects s ON sr.subject_id = s.id WHERE sr.student_id = ? AND sr.session_id = ? AND sr.term_id = ? AND sr.is_published = 1 ORDER BY s.subject_name");
