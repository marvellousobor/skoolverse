    ...
    if ($class_id <= 0 || $session_id <= 0 || $term_id <= 0) {
        ...
        if ($file_ext != 'csv') {
            ...
            try {
                ...
                // Detect format: old (single score) or new (CA + Exam per subject)
                $is_ca_exam_format = false;
                foreach ($headers as $h) {
                    if (preg_match('/_CA$/i', trim($h)) || preg_match('/_Exam$/i', trim($h))) {
                        $is_ca_exam_format = true;
                        break;
                    }
                }
                ...
                for ($i = 1; $i < count($headers); $i++) {
                    ...
                    if ($is_ca_exam_format) {
                        $base = preg_replace('/_(CA|Exam)$/i', '', $col_name);
                        $type = preg_match('/_CA$/i', $col_name) ? 'ca' : (preg_match('/_Exam$/i', $col_name) ? 'exam' : 'single');
                        if (!isset($subject_ids[$base])) {
                            $subject_count = $conn->query("SELECT COUNT(*) as count FROM subjects WHERE is_active = 1")->fetch_assoc()['count'];
                            if ($subject_count >= 15) {
                                throw new Exception("Maximum 15 subjects allowed.");
                            }
                            $stmt = $conn->prepare("SELECT id FROM subjects WHERE LOWER(subject_name) = LOWER(?)");
                            $stmt->bind_param("s", $base);
                            $stmt->execute();
                            $subj = $stmt->get_result()->fetch_assoc();
                            if ($subj) {
                                $subject_ids[$base] = $subj['id'];
                            } else {
                                $stmt = $conn->prepare("INSERT INTO subjects (subject_name) VALUES (?)");
                                $stmt->bind_param("s", $base);
                                $stmt->execute();
                                $subject_ids[$base] = $conn->insert_id;
                            }
                        }
                        if ($type == 'ca') $ca_columns[$i] = ['name' => $base, 'id' => $subject_ids[$base]];
                        elseif ($type == 'exam') $exam_columns[$i] = ['name' => $base, 'id' => $subject_ids[$base]];
                        else $single_columns[$i] = ['name' => $base, 'id' => $subject_ids[$base]];
                    } else {
                        $subject_name = $col_name;
                        if (strlen($subject_name) > 100) {
                            throw new Exception("Subject name too long: " . htmlspecialchars($subject_name));
                        }
                        $subject_count = $conn->query("SELECT COUNT(*) as count FROM subjects WHERE is_active = 1")->fetch_assoc()['count'];
                        if ($subject_count >= 15) {
                        ...
                        if ($subject) {
                            $single_columns[$i] = ['name' => $subject_name, 'id' => $subject['id']];
                        } else {
                            $stmt = $conn->prepare("INSERT INTO subjects (subject_name) VALUES (?)");
                            $stmt->bind_param("s", $subject_name);
                            $stmt->execute();
                            $single_columns[$i] = ['name' => $subject_name, 'id' => $conn->insert_id];
                        }
                    }
                }

                $all_columns = empty($single_columns) ? [] : $single_columns;
                if (empty($all_columns) && ($is_ca_exam_format)) {
                    // Use ca_columns as the base subject list
                }
                if (empty($single_columns) && empty($ca_columns)) {
                ...
                while (($data = fgetcsv($handle, 1000, ',')) !== false) {
                    ...
                    if ($is_ca_exam_format) {
                        // Build unique subjects from ca + exam columns
                        $subjects_combined = [];
                        foreach ($ca_columns as $col => $info) {
                            $subjects_combined[$info['name']] = $info['id'];
                        }
                        foreach ($exam_columns as $col => $info) {
                            if (!isset($subjects_combined[$info['name']])) {
                                $subjects_combined[$info['name']] = $info['id'];
                            }
                        }
                        foreach ($single_columns as $col => $info) {
                            $subjects_combined[$info['name']] = $info['id'];
                        }

                        foreach ($subjects_combined as $subj_name => $subject_id) {
                            $ca_val = null;
                            $exam_val = null;
                            // Find CA column
                            foreach ($ca_columns as $col => $info) {
                                if ($info['name'] === $subj_name) {
                                    $raw = isset($data[$col]) ? trim($data[$col]) : '';
                                    if ($raw !== '' && strtolower($raw) !== 'n/a' && strtolower($raw) !== 'absent' && is_numeric($raw)) {
                                        $ca_val = (float)$raw;
                                        if ($ca_val < 0) $ca_val = 0;
                                        if ($ca_val > 40) $ca_val = 40;
                                    }
                                    break;
                                }
                            }
                            ...
                            foreach ($exam_columns as $col => $info) {
                                if ($info['name'] === $subj_name) {
                                    ...
                                    if ($raw !== '' && strtolower($raw) !== 'n/a' && strtolower($raw) !== 'absent' && is_numeric($raw)) {
                                        $exam_val = (float)$raw;
                                        if ($exam_val < 0) $exam_val = 0;
                                        if ($exam_val > 70) $exam_val = 70;
                                    }
                                    break;
                                }
                            }
                            ...
                            foreach ($single_columns as $col => $info) {
                                if ($info['name'] === $subj_name) {
                                    ...
                                    if ($raw !== '' && strtolower($raw) !== 'n/a' && strtolower($raw) !== 'absent' && is_numeric($raw)) {
                                        ...
                                        // Use single total if CA/Exam not provided
                                        if ($ca_val === null && $exam_val === null) {
                                            $ca_val = null;
                                            $exam_val = null;
                                        }
                                    }
                            ...
                            if ($ca_val === null && $exam_val === null) continue;

                            $total = ($ca_val ?: 0) + ($exam_val ?: 0);
                            ...
                            if (isset($teacher_id) && $teacher_id > 0) {
                                ...
                                $stmt->bind_param("iiiiidddi", $student_id, $class_id, $session_id, $term_id, $subject_id, $total, $ca_val, $exam_val, $teacher_id);
                            } else {
                                $stmt = $conn->prepare("INSERT INTO student_results (student_id, class_id, session_id, term_id, subject_id, score, ca_score, exam_score, uploaded_by_teacher_id, is_published, published_by_admin_id, published_at) 
                                ...
                                $stmt->bind_param("iiiiiddd", $student_id, $class_id, $session_id, $term_id, $subject_id, $total, $ca_val, $exam_val);
                            }
                            $stmt->execute();
                            $total_results++;
                        }
                    } else {
                        // Old format: single score per subject
                        foreach ($single_columns as $col_index => $info) {
                            $subject_id = $info['id'];
                            $score = isset($data[$col_index]) ? trim($data[$col_index]) : '';
                            if ($score === '' || $score === 'N/A' || strtolower($score) === 'absent') continue;
                            if (!is_numeric($score)) {
                            ...
                            if ($score < 0 || $score > 100) {
                                $warnings[] = "Row $row_num: Score must be between 0 and 100. Got $score. Adjusted.";
                                $score = min(100, max(0, $score));
                            }
                            if (isset($teacher_id) && $teacher_id > 0) {
                                $stmt = $conn->prepare("INSERT INTO student_results (student_id, class_id, session_id, term_id, subject_id, score, uploaded_by_teacher_id, is_published, published_by_admin_id, published_at) 
                                                      VALUES (?, ?, ?, ?, ?, ?, ?, 0, NULL, NULL)
                                                      ON DUPLICATE KEY UPDATE score = VALUES(score), uploaded_by_teacher_id = VALUES(uploaded_by_teacher_id), is_published = 0, published_by_admin_id = NULL, published_at = NULL");
                                $stmt->bind_param("iiiiidi", $student_id, $class_id, $session_id, $term_id, $subject_id, $score, $teacher_id);
                            } else {
                                $stmt = $conn->prepare("INSERT INTO student_results (student_id, class_id, session_id, term_id, subject_id, score, uploaded_by_teacher_id, is_published, published_by_admin_id, published_at) 
                                                      VALUES (?, ?, ?, ?, ?, ?, NULL, 0, NULL, NULL)
                                                      ON DUPLICATE KEY UPDATE score = VALUES(score), uploaded_by_teacher_id = NULL, is_published = 0, published_by_admin_id = NULL, published_at = NULL");
                                $stmt->bind_param("iiiiid", $student_id, $class_id, $session_id, $term_id, $subject_id, $score);
                            }
                            $stmt->execute();
                            $total_results++;
                        }
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div>
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
                ...
                <div class="mb-6">
                    <h4 class="font-semibold text-blue-900 mb-2">Format 1: CA + Exam (recommended)</h4>
                    <div class="bg-white p-3 rounded text-xs text-gray-700 font-mono mb-2">
Student Name,Math_CA,Math_Exam,English_CA,English_Exam<br/>
John Doe,30,55,28,60<br/>
Jane Smith,32,58,27,62
                    </div>
                    <p class="text-sm text-blue-800 mt-1">CA: 0-40 | Exam: 0-70 | Total computed automatically</p>
                </div>

                <div class="mb-6">
                    <h4 class="font-semibold text-blue-900 mb-2">Format 2: Single Total Score</h4>
                    <div class="bg-white p-3 rounded text-xs text-gray-700 font-mono mb-2">
...
Jane Smith,90,88,95
                ...
                <div class="mb-4 p-3 bg-white rounded text-sm text-gray-700">
                    ...
                    <ul class="mt-2 space-y-1 text-xs">
                        ...
                        <li>✓ Subjects not assigned to this class via <strong>Classes → Class Subjects</strong> will still be auto-created</li>
                    </ul>
                </div>
