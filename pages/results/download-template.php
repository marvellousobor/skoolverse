...
if ($class_id <= 0) {
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="max-w-2xl mx-auto bg-white rounded-lg shadow p-8">
            ...
            <form method="GET" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Template Format</label>
                    <div class="flex gap-4 mb-3">
                        <label class="flex items-center gap-2">
                            <input type="radio" name="format" value="ca_exam" checked>
                            <span class="text-gray-700">CA + Exam (recommended)</span>
                        </label>
                        <label class="flex items-center gap-2">
                            <input type="radio" name="format" value="single">
                            <span class="text-gray-700">Single Total Score</span>
                        </label>
                    </div>
                </div>
    ...
    // Determine format
    $format = isset($_GET['format']) ? $_GET['format'] : 'ca_exam';

    // Create CSV
    $suffix = $format === 'ca_exam' ? '_ca_exam' : '';
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="results_' . preg_replace('/[^a-z0-9]/i', '_', $class['class_name']) . $suffix . '_template.csv"');
    ...
    if ($format === 'ca_exam') {
        ...
        foreach ($subject_list as $subject) {
            $headers[] = $subject['subject_name'] . '_CA';
            $headers[] = $subject['subject_name'] . '_Exam';
        }
    } else {
        $headers = ['Student Name'];
        foreach ($subject_list as $subject) {
