...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($login_type == ROLE_ADMIN || $login_type == ROLE_PARENT || $login_type == ROLE_TEACHER) {
        ...
        if ($login_type == ROLE_ADMIN) {
            $sql = "SELECT * FROM users WHERE email = ? AND (role = 'admin' OR role = 'super-admin') AND status = 'active'";
        } else {
    ...
    if ($user) {
        ...
        header("Location: " . get_role_home_url($user['role'] ?? ROLE_ADMIN));
        exit;
    }
}
?>
