    ...
    if ($check_stmt->get_result()->num_rows > 0) {
        ...
        if ($check_stmt->get_result()->num_rows > 0) {
            ...
            if (!$stmt->execute()) {
                ...
                if ($parent_id !== NULL) {
                    $sql = "INSERT INTO students (user_id, admission_no, first_name, last_name, middle_name, gender, date_of_birth, class_id, department, session_id, parent_id, phone, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("issssssissis", $user_id, $admission_no, $first_name, $last_name, $middle_name, $gender, $date_of_birth, $class_id, $department, $session_id, $parent_id, $phone);
                } else {
                    $sql = "INSERT INTO students (user_id, admission_no, first_name, last_name, middle_name, gender, date_of_birth, class_id, department, session_id, phone, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("issssssissi", $user_id, $admission_no, $first_name, $last_name, $middle_name, $gender, $date_of_birth, $class_id, $department, $session_id, $phone);
                }
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid-2">
                            <div class="form-group">
                                ...
                                <select name="class_id" required class="form-control" id="class_select" onchange="toggleDepartmentField()">
...
<script>
// Class level lookup for department toggle
...
function toggleDepartmentField() {
    ...
    const deptSelect = deptGroup.querySelector('select');
    const level = classLevels[sel.value] || '';
    if (level.toLowerCase().includes('senior')) {
