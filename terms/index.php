...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    // Check if term has associated fees or results
    $check = $conn->query("SELECT COUNT(*) as count FROM class_fees WHERE term_id = $term_id");
    $result = $check->fetch_assoc();
    
    if ($result['count'] > 0) {
        $error = "Cannot delete term with associated fees. Remove term from fees first.";
    } else {
        $check = $conn->query("SELECT COUNT(*) as count FROM results WHERE term_id = $term_id");
        $result = $check->fetch_assoc();
