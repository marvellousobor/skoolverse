    ...
    if ($result_id > 0) {
        if ($score === null) {
            ...
            if ($score < 0 || $score > 100) {
                ...
                $stmt = $conn->prepare("UPDATE student_results SET score = ? WHERE id = ? AND class_id = ? AND session_id = ? AND term_id = ?");
                $stmt->bind_param("diiii", $score, $result_id, $class_id, $session_id, $term_id);
                if ($stmt->execute()) {
...
// Get all subjects with results for this class/session/term
$subjects = $conn->query("
    SELECT DISTINCT sr.subject_id, s.subject_name
...
// Get all results
$results = $conn->query("
    SELECT sr.id, sr.student_id, sr.subject_id, sr.score, st.subject_name
    FROM student_results sr
                ...
                <thead class="bg-gray-100 border-b">
                    <tr>
                        ...
                        <?php foreach ($subjects as $subject): ?>
                            <th class="px-4 py-3 text-center text-sm font-semibold text-gray-900 bg-indigo-50">
                                ...
                                <div class="text-xs text-gray-600"><?php echo htmlspecialchars($subject['subject_name']); ?></div>
                            </th>
                        <?php endforeach; ?>
                                    ...
                                    <?php if (isset($results_map[$student['id']][$subject['subject_id']])): ?>
                                        ...
                                        <form method="POST" class="inline flex items-center gap-1">
                                            ...
                                            <input type="number" 
                                                   ...
                                                   max="100" 
                                                   step="0.01"
                                                   class="w-16 px-2 py-1 border border-gray-300 rounded text-center text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                            <button type="submit" title="Save" class="text-green-600 hover:text-green-800 font-bold">✓</button>
                                        </form>
                                    <?php else: ?>
